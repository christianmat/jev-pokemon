...
async function decideBattle(ctx: Ctx) {
  ...
  for (const it of gs.bag()) {
    ...
    if (/BALL$/.test(it.name) && b.kind === 'wild') {
      ...
      const weakest = [...party].sort((a, c) => a.level - c.level)[0];
      const beatsWeakest = weakest && b.enemy.level > weakest.level ? `It's higher level than your weakest team member (${weakest.nickname} Lv${weakest.level}). ` : '';
